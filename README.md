...
Knowledge endpoints use the same shared access model as the workforce API: no
sign-in or Clerk keys are required. Users can generate, edit, approve, assign,
submit completion evidence, and verify live records. Audit events identify the
`shared-workspace` actor rather than individual users. Approval and completion
assessment remain explicit workflow steps.

Database outages receive clearly labeled sample profiles.
