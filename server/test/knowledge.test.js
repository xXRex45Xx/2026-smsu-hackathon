...
import { canManage, requireManager, knowledgeActor } from "../src/middleware/knowledge-auth.js";
...
test("knowledge requests use shared workforce access without a Clerk session", () => {
  const req = { headers: {} };
  let calls = 0;
  knowledgeActor(req, {}, () => calls++);
  assert.equal(calls, 1);
  assert.deepEqual(req.actor, { id: "shared-workspace", role: "admin" });
  assert.equal(canManage(req.actor), true);
  assert.doesNotThrow(() => requireManager(req.actor));
});
