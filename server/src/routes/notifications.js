...
const router = Router();
// The shared navbar shows organization-wide notices only, never employee-specific messages.
router.get("/organization", asyncRoute(async (req, res) => {
  const rows = await db.select().from(notifications).where(isNull(notifications.employeeId));
  res.json({ data: rows });
}));
