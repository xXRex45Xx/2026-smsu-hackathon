...
const router = Router();
router.get("/dashboard", asyncRoute(async (req, res) => { const snapshot = await getSnapshot(queryFilters(req)); res.json({ data: withSkillHeatmap(withFutureSkills({ ...snapshot.summary, gaps: snapshot.gaps, skills: snapshot.skills.slice(0, 8), successionRisks: snapshot.successionRisks.slice(0, 5) }, snapshot), snapshot) }); }));
