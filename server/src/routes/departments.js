...
router.post("/", asyncRoute(async (req, res) => { const body = z.object({ id: z.string().min(1).optional(), name: z.string().min(1) }).parse(req.body); const [department] = await db.insert(departments).values({ id: body.id || crypto.randomUUID(), name: body.name }).returning(); res.status(201).json({ data: department }); }));
router.patch("/:departmentId", asyncRoute(async (req, res) => { const body = z.object({ name: z.string().min(1) }).parse(req.body); const [department] = await db.update(departments).set(body).where(eq(departments.id, req.params.departmentId)).returning(); if (!department) return res.status(404).json({ error: "Department not found" }); res.json({ data: department }); }));
