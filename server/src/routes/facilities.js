...
router.post("/", asyncRoute(async (req, res) => { const body = z.object({ id: z.string().min(1).optional(), name: z.string().min(1), location: z.string().optional() }).parse(req.body); const [facility] = await db.insert(facilities).values({ ...body, id: body.id || crypto.randomUUID() }).returning(); res.status(201).json({ data: facility }); }));
router.patch("/:facilityId", asyncRoute(async (req, res) => { const body = z.object({ name: z.string().min(1).optional(), location: z.string().optional() }).parse(req.body); const [facility] = await db.update(facilities).set(body).where(eq(facilities.id, req.params.facilityId)).returning(); if (!facility) return res.status(404).json({ error: "Facility not found" }); res.json({ data: facility }); }));
