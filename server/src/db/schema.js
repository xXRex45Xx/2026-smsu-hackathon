...
export const employeeSkills = pgTable("employee_skills", {
  ...
  yearsExperience: numeric("years_experience", { precision: 4, scale: 1 }).notNull(),
  verified: boolean("verified").default(false).notNull(),
}, (table) => [
  primaryKey({ columns: [table.employeeId, table.skillId] }),
  check("employee_skills_proficiency_check", sql`${table.proficiency} between 1 and 5`),
]);
...
export const roleSkillRequirements = pgTable("role_skill_requirements", {
  ...
  check("role_skill_required_level_check", sql`${table.requiredLevel} between 1 and 5`),
  check("role_skill_importance_check", sql`${table.importance} between 1 and 5`),
]);
...
export const futureSkillRequirements = pgTable("future_skill_requirements", {
  scenarioId: text("scenario_id").notNull().references(() => workforceScenarios.id, { onDelete: "cascade" }),
  skillId: text("skill_id").notNull().references(() => skills.id, { onDelete: "cascade" }),
  ...
  check("future_required_level_check", sql`${table.requiredLevel} between 1 and 5`),
]);
...
export const developmentPlanItems = pgTable("development_plan_items", {
  ...
  check("plan_item_current_level_check", sql`${table.currentLevel} between 1 and 5`),
  check("plan_item_target_level_check", sql`${table.targetLevel} between 1 and 5`),
]);
...
export const businessProcesses = pgTable("business_processes", {
  ...
  check("business_processes_employees_check", sql`${table.employeesInvolved} >= 0`),
...
export const processPainPoints = pgTable("process_pain_points", {
  ...
  check("process_pain_points_severity_check", sql`${table.severity} between 1 and 5`),
]);
...
export const aiUseCases = pgTable("ai_use_cases", {
  ...
  check("ai_use_cases_value_check", sql`${table.valueScore} between 0 and 100`),
  check("ai_use_cases_complexity_check", sql`${table.complexity} between 0 and 100`),
  check("ai_use_cases_risk_check", sql`${table.risk} between 0 and 100`),
]);

export const aiUseCaseSkillRequirements = pgTable("ai_use_case_skill_requirements", {
  useCaseId: text("use_case_id").notNull().references(() => aiUseCases.id, { onDelete: "cascade" }),
  skillId: text("skill_id").notNull().references(() => skills.id, { onDelete: "cascade" }),
  requiredLevel: integer("required_level").notNull(),
  requiredPeople: integer("required_people").notNull(),
}, (table) => [
  primaryKey({ columns: [table.useCaseId, table.skillId] }),
  check("use_case_required_level_check", sql`${table.requiredLevel} between 1 and 5`),
...
export const learningCourses = pgTable("learning_courses", {
  ...
  demoEnrollmentCount: integer("demo_enrollment_count").default(0).notNull(),
});

export const courseEnrollments = pgTable("course_enrollments", {
  courseId: text("course_id").notNull().references(() => learningCourses.id, { onDelete: "cascade" }),
  employeeId: text("employee_id").notNull().references(() => employees.id, { onDelete: "cascade" }),
  status: text("status").default("ENROLLED").notNull(),
  enrolledAt: timestamp("enrolled_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [
  primaryKey({ columns: [table.courseId, table.employeeId] }),
]);
